@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="mt-7 flex items-center justify-between">
        <div class="flex justify-between flex-1 sm:hidden">
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-green-500 bg-white border border-green-300 cursor-default leading-5 rounded-md dark:text-green-600 dark:bg-green-800 dark:border-green-600">
            ...
            @else
                <span class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-medium text-green-500 bg-white border border-green-300 cursor-default leading-5 rounded-md dark:text-green-600 dark:bg-green-800 dark:border-green-600">
                    {!! __('pagination.next') !!}
                </span>
            @endif
        </div>
                ...
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-green-500 bg-white border border-green-300 cursor-default rounded-l-md leading-5 dark:bg-green-800 dark:border-green-600" aria-hidden="true">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-green-700 bg-white border border-green-300 cursor-default leading-5 dark:bg-green-800 dark:border-green-600">{{ $element }}</span>
                            </span>
                        @endif
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-green-500 bg-white border border-green-300 cursor-default leading-5 dark:bg-green-800 dark:border-green-600">{{ $page }}</span>
                                    </span>
                                @else
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            <span class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-green-500 bg-white border border-green-300 cursor-default rounded-r-md leading-5 dark:bg-green-800 dark:border-green-600" aria-hidden="true">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
                            </span>
